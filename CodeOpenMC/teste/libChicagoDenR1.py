...
class ChigagoDenR1:
    ...
    def tallies(self,):
        ...
        energy_filter_fast = openmc.EnergyFilter([1.0    , 20E06]) 
        ...
        energy = np.logspace(-5, np.log10(14.1), num=100)
        ...
        #energy = np.logspace(-5, 8, num=101)
        #r_divisions_radial = [122/2, (122/2)+1]  
        #z_divisions_radial = [self.centro_fonte-10, self.centro_fonte+10.0]
        #mesh_espectro_central = openmc.CylindricalMesh(r_grid = (r_divisions_radial), z_grid = (z_divisions_radial))
        #mesh_filter_espectro_central = openmc.MeshFilter(mesh_espectro_central)
        #energy_filter = openmc.EnergyFilter(energy)
        #fuel_element_tally = openmc.Tally(name='Fluxo no universo combustível',tally_id=3) # F34
        #fuel_element_tally.filters.append(energy_filter) #openmc.CellFilter(self.celula_combustivel)
        #fuel_element_tally.filters.append(mesh_filter_espectro_central)
        ...
        ###############################################################################################################
        divisions_rc = 566
        x_divisions_rc = np.linspace(0,self.fronteira_ar_lateral,divisions_rc).tolist()
        y_divisions_rc = [-1.47/2, 1.47/2]
        ...
        if self.fonte:
            ...
            amplitude_thermal = self.atividade*flux_cub_thermal.reshape((divisions_cubico-1, divisions_cubico-1))/V
            amplitude_thermal_dev = self.atividade*flux_cub_thermal_dev.reshape((divisions_cubico-1, divisions_cubico-1))/V
            ...
            V = (x_divisions[1]-x_divisions[0])*(y_divisions[1]-y_divisions[0])*(z_divisions[1]-z_divisions[0])
            amplitude_fast = self.atividade*flux_cub_fast.reshape((divisions_cubico-1, divisions_cubico-1))/V
            amplitude_fast_dev = self.atividade*flux_cub_fast_dev.reshape((divisions_cubico-1, divisions_cubico-1))/V
            ...
            print()
            # Fluxo em intervalo térmico 
            for i in range(0,divisions_rc-1):
